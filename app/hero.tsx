...
export const SLIDE_DURATION = 30_000;
...
export function Hero(){
  ...
  useEffect(() => {
    if (!api || paused || pageHidden || focused) return;
    const timer = window.setTimeout(() => api.scrollNext(), SLIDE_DURATION);
    return () => window.clearTimeout(timer);
  }, [api, active, paused, pageHidden, focused]);
  ...
  return <Carousel className="hero hero-modern" opts={{loop:true, duration:35}} setApi={setApi} aria-label="Fotografitë e auto sportit të Kosovës">
    <CarouselContent className="hero-slides">{slides.map((slide,index) => <CarouselItem className="hero-slide" key={slide.image}><img src={slide.image} alt={slide.alt} className="hero-photo" style={{objectPosition:slide.position}} fetchPriority={index===0?'high':'auto'} loading={index===0?'eager':'lazy'}/></CarouselItem>)}</CarouselContent>
    <div className="hero-shade"/>
    <div className="container hero-content">
      <div className="eyebrow"><span/>FEDERATA E AUTO SPORTIT E KOSOVËS</div>
      <h1>NJË PASION.<br/>NJË <span>KOMB.</span><br/>PA KUFIJ.</h1>
      <p>Bashkë në çdo kthesë.<br/>Drejt çdo fitoreje.</p>
      <div className="hero-actions"><a href="/kalendari/" className="button yellow-button">Zbulo sezonin 2026 <ArrowUpRight size={21}/></a><a href="#lajmet" className="button hero-secondary">Lajmet e fundit <ArrowRight size={20}/></a></div>
    </div>
